...
static void bt_function_select_init()
{
    ...
    __bt_set_update_battery_time(60); /*设置电量显示发送更新的周期时间，为0表示关闭电量显示功能，单位秒，u32, 不能小于10s*/
    ...
    __set_device_role(S_DEVICE_ROLE_SLAVE | M_DEVICE_ROLE_MASTER); /*设置设备role*/
#else
...
static void set_device_volume(int volume)
{
    ...
    if (get_tone_status() || get_esco_busy_flag() || (get_is_in_background_flag())) {
        puts("It's not smart to sync a2dp vol now\n");
        return;
    }

    sound.vol.sys_vol_l = (volume *  MAX_SYS_VOL_L / 127) ;
#if BT_TWS
    ...
    if (sound.vol.sys_vol_l != sound.vol.sys_vol_r) {
