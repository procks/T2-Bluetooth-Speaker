...
struct user_ctrl {
    ...
    struct sys_timer bt_phone_vol_timeout;
...
void phone_sync_vol_timer(struct sys_timer *ts)
{
    //puts("phone_sync_vol_timer\n");
    user_send_cmd_prepare(USER_CTRL_HFP_CALL_SET_VOLUME, 1, &sound.phone_vol);
    user_send_cmd_prepare(USER_CTRL_HFP_CALL_VOLUME_UP, 0, NULL);
    sys_timer_remove(&user_val->bt_phone_vol_timeout);
}
...
#endif
        if (!is_1t2_connection()) { //回连下一个device
            puts("conn_next_dev\n");
            user_send_cmd_prepare(USER_CTRL_START_CONNECTION, 0, NULL);
        }
        break;
...
void tws_sync_connect_deal()
{
    ...
    task_post_msg(NULL, 1, MSG_BT_TONE_CONN);//主从、连接上sync play tone
...
void bt_tws_sync_connect_deal(u8 conn_role)
{
    ...
    while (wait_try_cnt) {
        ...
        if (conn_role == BT_CUR_CONN_TWS_SLAVE) { //主机send sync_pany_tone
            if (send_sync_tws_cmd_fun(TWS_SYNC_PLAY_TONE, TWS_SYNC_PLAY_TONE_SLOT, tws_sync_connect_deal)) {
                return;
            }
...
/* void task_bt_deal(void) */
{
    ...
    while (1) {
        ...
        case MSG_BT_CALL_LAST_NO:
            ...
            break;
        case MSG_BT_PHONE_VOL:
            task_bt_puts("MSG_BT_PHONE_VOL\n");
            sys_timer_register(&user_val->bt_phone_vol_timeout, 1200, phone_sync_vol_timer, 1);
            break;
