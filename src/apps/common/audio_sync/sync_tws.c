...
static u8 twsSyncBuf[3200] sec(.tws_sync_buf_sec);
...
static void tws_sync_init(u16 ch, u16 sr)
{
    ...
    if (tws_sync.tws_sync_parm.custom[TWS_CUSTOM_ASB_LEN_FLAG] && (tws_sync.tws_sync_parm.set_ch_mode == 0)) {
...
static u32 tws_sync_run(void *priv, void *buf, u32 len)
{
    ...
    if (tws_sync.toggle) {
        ...
        res = tws_sync_ops->run(twsSyncBuf, buf, len, tws_sync.tws_sync_parm.tws_ps_flag);
        tws_sync.tws_sync_parm.tws_ps_flag = 0;
        ...
        if (tws_sync.tws_sync_parm.pre_packet_pcm_cnt && (tws_sync.tws_sync_parm.dec_packet_pcm_cnt == tws_sync.tws_sync_parm.pre_packet_pcm_cnt)) {
...
{
    u16 tws_sbc_data_variable_min = (3 * 1024 - 300); //最低门限值进行变包设置，不能设置那么大，而影响音质，也不能设置那么小，会影响距离
    u16 tws_sbc_data_variable_max = (7 * 1024 - 300);
    ...
    tws_sync.tws_sync_parm.tws_music_decoder_loop_resume  = music_decoder_loop_resume ;
