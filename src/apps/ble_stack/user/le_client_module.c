...
#define ACT_SET_FLAG(a)       {act_operate_control |= BIT(a); client_resume();}
...
//----------------------------------------------------------------------------
static void client_resume(void);
...
static void client_thread_process(void)
{
    ...
    if (ACT_CHECK_FALG(CLI_DIS_CONNECT)) {
        ACT_CLR_FLAG(CLI_DIS_CONNECT);
        client_disconnect(gc_handle);
    }

    if (ACT_CHECK_FALG(CLI_SEARCH_PROFILE_START)) {
        ACT_CLR_FLAG(CLI_SEARCH_PROFILE_START);
        client_search_profile_start(PFL_SERVER_ALL, 0);
    }

    if (ACT_CHECK_FALG(CLI_SCAN_ENABLE)) {
        ACT_CLR_FLAG(CLI_SCAN_ENABLE);
        client_scan_enable(1);
    }

    if (ACT_CHECK_FALG(CLI_SCAN_DISABLE)) {
        ACT_CLR_FLAG(CLI_SCAN_DISABLE);
