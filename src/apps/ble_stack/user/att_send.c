...
static u8 att_ready_send_len;
static u16 att_ready_send_handle;
static u8 *att_ready_send_buf;

static u8   att_send_data[248] __attribute__((aligned(4)));//need >= 20
...
void (*att_thread_deal_callback)(void) = NULL;
...
{
    if (att_thread_deal_callback) {
        att_thread_deal_callback();
    }
...
static void att_send_schedule(void)
{
    ...
    if (check_att_server_ready_send()) {
    } else {
        if (user_att_send_streamer()) {
        }
    }

    if (att_wakeup_send_callback) {
